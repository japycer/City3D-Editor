...
export class Octree {
    constructor(parameters) {
        ...
        this.INDEX_OUTSIDE_MAP[this.INDEX_OUTSIDE_NEG_X] =
            { index: this.INDEX_OUTSIDE_NEG_X, count: 0, x: -1, y: 0, z: 0 };
        this.INDEX_OUTSIDE_MAP[this.INDEX_OUTSIDE_POS_Y] =
            { index: this.INDEX_OUTSIDE_POS_Y, count: 0, x: 0, y: 1, z: 0 };
        this.INDEX_OUTSIDE_MAP[this.INDEX_OUTSIDE_NEG_Y] =
            { index: this.INDEX_OUTSIDE_NEG_Y, count: 0, x: 0, y: -1, z: 0 };
        this.INDEX_OUTSIDE_MAP[this.INDEX_OUTSIDE_POS_Z] =
            { index: this.INDEX_OUTSIDE_POS_Z, count: 0, x: 0, y: 0, z: 1 };
        this.INDEX_OUTSIDE_MAP[this.INDEX_OUTSIDE_NEG_Z] =
            { index: this.INDEX_OUTSIDE_NEG_Z, count: 0, x: 0, y: 0, z: -1 };
        ...
        this.depthMax =
            isNumber(parameters.depthMax) ? parameters.depthMax : Infinity;
        this.objectsThreshold =
            isNumber(parameters.objectsThreshold) ? parameters.objectsThreshold : 8;
        this.overlapPct =
            isNumber(parameters.overlapPct) ? parameters.overlapPct : 0.15;
        this.undeferred = parameters.undeferred || false;
        this.root = parameters.root instanceof OctreeNode ?
            parameters.root :
            new OctreeNode(parameters);
    };
        ...
        if (!this.objectsMap[object.uuid]) {
            ...
            if (useVertices === true) {
                ...
                for (i = 0, l = faces.length; i < l; i++) {
                    let face = faces[i];
                    let centroid = vertices[face.a].clone();
    ...
    growBounding() {
        this.root.growBounding();
    }

    remove(object) {
        let i, l, objectData = object, index, objectsDataRemoved;
        ...
        if (this.objectsMap[object.uuid]) {
            ...
            index =
                indexOfPropertyWithValue(this.objectsDeferred, 'object', object);
            if (index !== -1) {
    ...
    extend(octree) {
        ...
        if (octree instanceof Octree) {
            ...
            for (i = 0, l = objectsData.length; i < l; i++) {
                ...
                this.add(
                    objectData,
                    { useFaces: objectData.faces, useVertices: objectData.vertices });
            }
        }
    }
    rebuild() {
        ...
        for (i = 0, l = this.objectsData.length; i < l; i++) {
            ...
            if (node instanceof OctreeNode &&
                !objectData.positionLast.equals(objectData.position)) {
    ...
    updateObject(object) {
        let i, l, parentCascade = [object], parent, parentUpdate;
    ...
    search(position, radius, organizeByObject, direction) {
        let i, l, node, objects, objectData, object, results, resultData,
            resultsObjectsIndices, resultObjectIndex, directionPct;
        // add root objects
        ...
        if (organizeByObject === true) {
            ...
            for (i = 0, l = objects.length; i < l; i++) {
                ...
                if (resultObjectIndex === -1) {
                    resultData = { object: object, faces: [], vertices: [] };
    ...
    findClosestVertex(position, radius) {
        ...
        let object = search[0].object, vertices = search[0].vertices;
        if (vertices.length === 0) {
            return null;
        }
        let distance, vertex = null,
    ...
    getDepthEnd() { return this.root.getDepthEnd(); }
    getNodeCountEnd() { return this.root.getNodeCountEnd(); }
    getObjectCountEnd() { return this.root.getObjectCountEnd(); }
    toConsole() { this.root.toConsole(); }
}
...
class OctreeObjectData {
    ...
    update() {
        if (this.face3) {
            ...
            this.position.copy(this.faces.centroid)
            ...
            if (this.object.geometry) {
                ...
                this.radius = this.object.boundRadius || 0;
    ...
    getFace3BoundingRadius(object, face) {
...
export class OctreeNode {
    constructor(parameters) {
        ...
        this.boundingX = this.radius;
        this.boundingY = this.radius;
        this.boundingZ = this.radius;
        // visual
        if (this.tree.scene) {
            // this.visual =
            ...
            const box = new Box3(new Vector3(center.x - this.boundingX, center.y - this.boundingY, center.z - this.boundingZ),
                new Vector3(center.x + this.boundingX, center.y + this.boundingY, center.z + this.boundingZ));
            this.visual = new THREE.Box3Helper(box, getIndexedRandomColor(this.depth));

            // this.visual.scale.set(this.radiusOverlap * 2, this.radiusOverlap * 2,
            //     this.radiusOverlap * 2);
            // this.visual.scale.set(this.boundingX * 2, this.boundingY * 2,
            //     this.boundingZ * 2);
            // this.visual.position.copy(this.position);
            this.tree.helperGroup.add(this.visual);
        }
    }

    growBounding() {
        // return
        let nminX = this.position.x - this.radius;
        let nmaxX = this.position.x + this.radius;
        let nminY = this.position.y - this.radius;
        let nmaxY = this.position.y - this.radius;
        let nminZ = this.position.z - this.radius;
        let nmaxZ = this.position.z - this.radius;

        if (this.nodesIndices.length) {

            for (let i of this.nodesIndices) {
                const node = this.nodesByIndex[i];
                let [minX, maxX, minY, maxY, minZ, maxZ] = node.growBounding();
                if (minX < nminX) {
                    nminX = minX;
                }
                if (maxX > nmaxX) {
        ...
        this.boundingX = Math.max(this.position.x - nminX, nmaxX - this.position.x);
        this.boundingY = Math.max(this.position.y - nminY, nmaxY - this.position.y);
        this.boundingZ = Math.max(this.position.z - nminZ, nmaxZ - this.position.z);

        if (this.tree.scene) {
            this.visual.box.min.x = this.position.x - this.boundingX;
            this.visual.box.max.x = this.position.x + this.boundingX;
            this.visual.box.min.y = this.position.y - this.boundingY;
            this.visual.box.max.y = this.position.y + this.boundingY;
            this.visual.box.min.z = this.position.z - this.boundingZ;
            this.visual.box.max.z = this.position.z + this.boundingZ;
        }
    ...
    reset(cascade, removeVisual) {
        let i, l, node, nodesIndices = this.nodesIndices || [],
    ...
    addObject(object) {
        ...
        if (indexOctant > -1 && this.nodesIndices.length > 0) {
            ...
            // debugger;
            // setTimeout(this.checkGrow.bind(this));
            this.checkGrow();
        }
    }
    ...
    removeObject(object) {
        let i, l, nodesRemovedFrom, removeData;
    ...
    removeObjectRecursive(object, removeData) {
        let i, l, index = -1, objectData, node, objectRemoved;
    ...
    checkGrow() {
        ...
        if (this.objects.length > this.tree.objectsThreshold &&
            this.tree.objectsThreshold > 0) {
            this.grow();
        }
    }
    grow() {
        let indexOctant, object, objectsExpand = [], objectsExpandOctants = [],
            objectsSplit = [], objectsSplitOctants = [],
            objectsRemaining = [], i, l;
        // for each object
        ...
        if (objectsSplit.length > 0) {
            objectsRemaining = objectsRemaining.concat(
                this.split(objectsSplit, objectsSplitOctants));
        }
        // if has objects to expand
        if (objectsExpand.length > 0) {
            objectsRemaining = objectsRemaining.concat(
                this.expand(objectsExpand, objectsExpandOctants));
        }
        // store remaining
    ...
    split(objects, octants) {
        let i, l, indexOctant, object, node, objectsRemaining;
        // if not at max depth
    ...
    branch(indexOctant) {
        let node, overlap, radius, radiusOffset, offset, position;
        // node exists
        if (this.nodesByIndex[indexOctant] instanceof OctreeNode) {
            ...
            offset = this.utilVec31Branch.set(
    ...
    expand(objects, octants) {
        let i, l, object, objectsRemaining, objectsExpand, indexOctant,
            ...
            infoPotential2, infoPotential3, indexPotentialBitwise1,
            indexPotentialBitwise2, octantX, octantY, octantZ, overlap, radius,
            radiusOffset, radiusParent, overlapParent,
            offset = this.utilVec31Expand, position, parent;
        ...
        if (this.tree.root.getDepthEnd() < this.tree.depthMax) {
            ...
            if (objectsExpand.length > 0) {
                ...
                // second is ( one of next two bitwise OR 1 ) that is not opposite of
                // ( first bitwise OR 1 )
                ...
                octantY =
                    infoIndexOutside1.y + infoIndexOutside2.y + infoIndexOutside3.y;
                octantZ =
                    infoIndexOutside1.z + infoIndexOutside2.z + infoIndexOutside3.z;
                // get this octant indices based on octant normal
                indexOctant =
                    this.getOctantIndexFromPosition(octantX, octantY, octantZ);
                indexOctantInverse =
                    this.getOctantIndexFromPosition(-octantX, -octantY, -octantZ);
                // properties
                ...
                parent = new OctreeNode(
                    { tree: this.tree, position: position, radius: radiusParent });
    ...
    checkMerge() {
        let nodeParent = this, nodeMerge;
        // traverse up tree as long as node + entire subtree's object count is
        // under minimum
        while (nodeParent.parent instanceof OctreeNode &&
            nodeParent.getObjectCountEnd() < this.tree.objectsThreshold) {
    ...
    checkContract() {
        let i, l, node, nodeObjectsCount, nodeHeaviest, nodeHeaviestObjectsCount,
        ...
        if (this.nodesIndices.length > 0) {
            ...
            for (i = 0, l = this.nodesIndices.length; i < l; i++) {
                ...
                if (nodeHeaviest instanceof OctreeNode === false ||
                    nodeObjectsCount > nodeHeaviestObjectsCount) {
                    nodeHeaviest = node;
            ...
            // if should contract
            if (outsideHeaviestObjectsCount < this.tree.objectsThreshold &&
                nodeHeaviest instanceof OctreeNode) {
    ...
    getOctantIndex(objectData) {
        ...
        // if outside, use bitwise flags to indicate on which sides object is
        // outside of
    ...
    intersectSphere(position, radius) {
        let distance = radius * radius, px = position.x, py = position.y,
            pz = position.z;
        if (px < this.left) {
    ...
    getNodeCountRecursive() {
        ...
        for (i = 0, l = this.nodesIndices.length; i < l; i++) {
            count +=
                this.nodesByIndex[this.nodesIndices[i]].getNodeCountRecursive();
        }
        return count;
    }
    ...
    getObjectCountStart() {
        let count = this.objects.length, parent = this.parent;
...
class OctreeRaycaster extends THREE.Raycaster {
    ...
    intersectOctreeObjects(objects,
        recursive) {
        let i, il, intersects = [];
